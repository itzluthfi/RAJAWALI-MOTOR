...
<div class="space-y-6 -m-3 p-3">
    ...
    {{-- SECTION 1: PERBAIKAN DATA & PEMELIHARAAN (ATAS) --}}
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <x-card class="flex flex-col justify-between shadow-lg border border-slate-200/80">
            ...
            <form method="POST" action="{{ route('utility.recalculate-stok') }}" onsubmit="return confirm('Apakah Anda yakin ingin memproses hitung ulang stok?')">
                ...
                <x-button type="submit" variant="primary" class="w-full">
                    ...
                    <span>Mulai Hitung Ulang Stok</span>
                </x-button>
            </form>
        </x-card>
        ...
        <x-card class="flex flex-col justify-between shadow-lg border border-slate-200/80">
            ...
            <form method="POST" action="{{ route('utility.maintenance-hpp') }}" onsubmit="return confirm('Apakah Anda yakin ingin memproses maintenance HPP?')">
                ...
                <x-button type="submit" variant="primary" class="w-full">
                    ...
                    <span>Mulai Maintenance HPP</span>
                </x-button>
            </form>
        </x-card>

        {{-- IMPORT BARANG --}}
        <x-card class="shadow-lg border border-slate-200/80">
            <div class="flex items-center gap-3 mb-3 text-rajawali">
            ...
            <p class="text-xs text-steel leading-relaxed mb-4 font-semibold">
                ...
                <code class="font-mono text-rajawali bg-rajawali/5 px-1 py-0.5 rounded text-[11px]">kode, nama, hpp, harga_eceran, harga_grosir, stok_minimum</code>
            </p>
        ...
        <x-card class="shadow-lg border border-slate-200/80">
            ...
            <p class="text-xs text-steel leading-relaxed mb-4 font-semibold">
                ...
                <code class="font-mono text-rajawali bg-rajawali/5 px-1 py-0.5 rounded text-[11px]">nama, telepon, alamat, termin_hari</code>
            </p>
    ...
    {{-- SECTION 2: PENCADANGAN DATABASE (PALING BAWAH) --}}
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
                <div class="flex items-center gap-2 mb-1">
                    <x-icon name="database" class="w-6 h-6 text-emerald-400" />
                    <h3 class="font-display font-black text-xl text-white tracking-tight">Pencadangan Database (Backup &amp; Riwayat Berkas)</h3>
                </div>
                ...
                <div class="mt-2.5 flex items-center gap-2 text-[11px] text-emerald-300 bg-emerald-950/60 border border-emerald-700/50 px-3 py-1.5 rounded-lg w-fit">
                    <x-icon name="clock" class="w-3.5 h-3.5 text-emerald-400 shrink-0" />
                    <span><strong>Jadwal Otomatis:</strong> Setiap hari pukul 23:59 WIB (Menyimpan riwayat 14 hari terakhir)</span>
                </div>
            </div>
            <form method="POST" action="{{ route('utility.backup') }}" onsubmit="return confirm('Mulai proses pembuatan backup database sekarang?')">
                @csrf
                <x-button type="submit" variant="primary" class="bg-emerald-600 hover:bg-emerald-700 text-white font-bold px-5 py-3 rounded-xl shadow-lg flex items-center gap-2 shrink-0 cursor-pointer">
                                ...
                                <td class="px-4 py-2.5 text-center">
                                    <div class="flex items-center justify-center gap-2">
                                        ...
                                        <form method="POST" action="{{ route('utility.backup.delete', $b['filename']) }}" onsubmit="return confirm('Apakah Anda yakin ingin menghapus berkas backup {{ $b['filename'] }} dari server?')">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="px-2 py-1 rounded bg-red-600/80 hover:bg-red-600 text-white font-bold text-[11px] inline-flex items-center gap-1 transition cursor-pointer" data-tooltip="Hapus Berkas dari Server">
