...
<head>
    ...
    <title>Layanan Kelistrikan & ECU | Rajawali Motor Surabaya</title>
    <meta name="description" content="Perbaikan sistem kelistrikan, diagnosa scanner ECU, altenator, starter, dan garansi penggantian aki original di Rajawali Motor Surabaya.">

    <!-- OpenGraph Social Preview Meta Tags -->
    <meta property="og:type" content="website"/>
    <meta property="og:url" content="https://rajawalimotor.com/layanan/kelistrikan"/>
    <meta property="og:title" content="Layanan Kelistrikan & ECU | Rajawali Motor Surabaya"/>
    <meta property="og:description" content="Perbaikan kelistrikan, diagnosa scanner ECU, dynamo starter & altenator di Surabaya."/>
...
<a href="https://wa.me/{{ $toko['whatsapp'] }}?text={{ urlencode('Halo Rajawali Motor, saya ingin booking kelistrikan...') }}" target="_blank" rel="noopener" aria-label="Chat via WhatsApp"
...
<nav class="fixed top-0 w-full z-50 glass-nav shadow-sm border-b border-outline-variant/20">
    <div class="flex justify-between items-center px-4 sm:px-8 py-3.5 max-w-7xl mx-auto">
        ...
        <div class="hidden lg:flex items-center gap-8 font-medium text-sm text-secondary">
            ...
            <a class="text-primary font-bold border-b-2 border-primary pb-0.5" href="{{ route('layanan.kelistrikan') }}">Kelistrikan</a>
            <a class="hover:text-primary transition-colors" href="{{ route('layanan.ac-mobil') }}">AC Mobil</a>
        </div>
        <div class="hidden sm:flex items-center gap-3">
            <a href="https://wa.me/{{ $toko['whatsapp'] }}?text={{ urlencode('Halo Rajawali Motor, saya ingin booking servis kelistrikan...') }}" target="_blank" rel="noopener"
...
<nav class="fixed inset-y-0 left-0 w-80 max-w-[85vw] rounded-r-2xl bg-surface z-[70] flex flex-col h-full p-6 -translate-x-full transition-transform duration-300 shadow-2xl overflow-y-auto" id="drawer">
    ...
    <div class="flex flex-col gap-2">
        ...
        <a class="flex items-center gap-3.5 p-3.5 bg-primary-container text-white font-semibold rounded-xl shadow-md" href="{{ route('layanan.kelistrikan') }}">
            <span class="material-symbols-outlined text-white">bolt</span> Kelistrikan & ECU
        </a>
    ...
    <div class="mt-auto pt-6 border-t border-outline-variant/30 flex flex-col gap-3">
        <a href="https://wa.me/{{ $toko['whatsapp'] }}?text={{ urlencode('Halo Rajawali Motor, saya ingin booking servis kelistrikan...') }}" target="_blank" rel="noopener" class="efek-kilau w-full bg-[#25D366] text-white text-center py-3.5 rounded-xl font-bold text-sm shadow-lg flex items-center justify-center gap-2 hover:bg-[#20bd5a] transition-all">
        ...
        <div class="relative z-10 px-4 sm:px-8 max-w-7xl mx-auto w-full py-16 sm:py-24 text-white">
            <div class="max-w-3xl space-y-6" data-reveal>
                <span class="inline-flex items-center gap-2 bg-primary-container px-4 py-1.5 rounded-full font-bold text-xs uppercase tracking-widest text-white">
                    <span class="material-symbols-outlined text-sm">bolt</span> Electrical & ECU Diagnostic
                </span>
                <h1 class="text-3xl sm:text-5xl lg:text-6xl font-extrabold leading-tight">Perbaikan Kelistrikan & Diagnosa ECU</h1>
                <p class="text-white/80 text-base sm:text-xl max-w-2xl leading-relaxed">Diagnosa akurat komputerisasi OBD-II untuk sistem pengisian aki, dynamo starter, pengapian, dan jaringan kabel elektrikal kendaraan.</p>
                <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-4 pt-4">
                    <a href="https://wa.me/{{ $toko['whatsapp'] }}?text={{ urlencode('Halo Rajawali Motor, saya ingin booking kelistrikan...') }}" target="_blank" rel="noopener"
    ...
    <section class="py-20 px-4 sm:px-8 bg-surface-container-low">
        <div class="max-w-7xl mx-auto">
            <div class="text-center max-w-2xl mx-auto mb-16" data-reveal>
                <p class="text-primary font-bold text-xs uppercase tracking-widest mb-2">PAKET KELISTRIKAN</p>
                <h2 class="font-bold text-2xl sm:text-4xl text-on-surface">Estimasi Biaya Transparan</h2>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8 items-stretch" data-reveal-stagger>
                <div data-reveal-item class="kartu-angkat efek-kilau bg-white p-8 rounded-3xl border border-outline-variant/40 shadow-sm flex flex-col justify-between">
                    <div>
                        <div class="flex justify-between items-start mb-6">
                            <div>
                                <h3 class="font-bold text-2xl text-on-surface">ECU Scan Diagnosa</h3>
                                <p class="text-secondary text-xs mt-1">Komputer OBD-II</p>
                            </div>
                            <div class="text-right">
                                <p class="text-primary font-bold text-2xl">Rp 90.000</p>
                            </div>
                        </div>
                        <ul class="space-y-3 mb-8 text-sm text-secondary">
                            <li class="flex items-center gap-2"><span class="material-symbols-outlined text-primary text-base">check</span> Full Diagnostic Error Code Check</li>
                            <li class="flex items-center gap-2"><span class="material-symbols-outlined text-primary text-base">check</span> Reset Indikator Check Engine</li>
                            <li class="flex items-center gap-2"><span class="material-symbols-outlined text-primary text-base">check</span> Cek Voltase Aki & Pengisian</li>
                        </ul>
                    </div>
                    <a href="https://wa.me/{{ $toko['whatsapp'] }}?text={{ urlencode('Saya berminat Paket ECU Scan Diagnosa') }}" target="_blank" rel="noopener"
                ...
                <div data-reveal-item class="kartu-angkat efek-kilau bg-inverse-surface p-8 rounded-3xl ring-4 ring-primary shadow-2xl relative flex flex-col justify-between">
                    ...
                    <div>
                        <div class="flex justify-between items-start mb-6 pt-2">
                            <div>
                                <h3 class="font-bold text-2xl text-white">Service Kelistrikan</h3>
                                <p class="text-primary-fixed-dim text-xs mt-1">Perbaikan Lampu & Pengapian</p>
                            </div>
                            <div class="text-right">
                                <p class="text-primary-fixed-dim font-bold text-2xl">Rp 180.000</p>
                            </div>
                        </div>
                        <ul class="space-y-3 mb-8 text-sm text-white/90">
                            <li class="flex items-center gap-2"><span class="material-symbols-outlined text-primary-fixed-dim text-base">verified</span> Perbaikan Kabel & Soket Longgar</li>
                            <li class="flex items-center gap-2"><span class="material-symbols-outlined text-primary-fixed-dim text-base">verified</span> Service Dynamo Starter / Altenator</li>
                            <li class="flex items-center gap-2"><span class="material-symbols-outlined text-primary-fixed-dim text-base">verified</span> Penggantian Sekring & Relai</li>
                        </ul>
                    </div>
                    <a href="https://wa.me/{{ $toko['whatsapp'] }}?text={{ urlencode('Saya berminat Paket Service Kelistrikan Standar') }}" target="_blank" rel="noopener"
                ...
                <div data-reveal-item class="kartu-angkat efek-kilau bg-white p-8 rounded-3xl border border-outline-variant/40 shadow-sm flex flex-col justify-between">
                    <div>
                        <div class="flex justify-between items-start mb-6">
                            <div>
                                <h3 class="font-bold text-2xl text-on-surface">Overhaul Rewiring</h3>
                                <p class="text-secondary text-xs mt-1">Peremajaan Kabel Bodi</p>
                            </div>
                            <div class="text-right">
                                <p class="text-primary font-bold text-2xl">Rp 350.000</p>
                            </div>
                        </div>
                        <ul class="space-y-3 mb-8 text-sm text-secondary">
                            <li class="flex items-center gap-2"><span class="material-symbols-outlined text-primary text-base">check</span> Urut Ulang Jalur Kabel Konslet</li>
                            <li class="flex items-center gap-2"><span class="material-symbols-outlined text-primary text-base">check</span> Perapian & Isolasi Tahan Panas</li>
                            <li class="flex items-center gap-2"><span class="material-symbols-outlined text-primary text-base">check</span> Kalibrasi ulang Sensor Mesin</li>
                            <li class="flex items-center gap-2"><span class="material-symbols-outlined text-primary text-base">check</span> Garansi Perbaikan Kelistrikan 30 Hari</li>
                        </ul>
                    </div>
                    <a href="https://wa.me/{{ $toko['whatsapp'] }}?text={{ urlencode('Saya berminat Paket Overhaul Rewiring') }}" target="_blank" rel="noopener"
                       class="w-full border-2 border-primary text-primary py-3.5 rounded-xl font-bold text-center block hover:bg-primary hover:text-white transition-colors">Pilih Paket</a>
                </div>
            </div>
        </div>
