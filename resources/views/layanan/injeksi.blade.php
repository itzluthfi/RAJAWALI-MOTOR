...
<head>
    ...
    <title>Layanan Spesialis Injeksi Desktop & Mobile | Rajawali Motor Surabaya</title>
    <meta name="description" content="Servis spesialis injeksi ultrasonik, throttle body, diagnosa ECU, dan kalibrasi sistem pembakaran di Rajawali Motor Surabaya.">

    <!-- OpenGraph Social Preview Meta Tags -->
    <meta property="og:type" content="website"/>
    <meta property="og:url" content="https://rajawalimotor.com/layanan/injeksi"/>
    <meta property="og:title" content="Layanan Spesialis Injeksi Desktop & Mobile | Rajawali Motor Surabaya"/>
    <meta property="og:description" content="Pusat servis injeksi ultrasonik, throttle body & kalibrasi ECU presisi di Surabaya."/>
...
<a href="https://wa.me/{{ $toko['whatsapp'] }}?text={{ urlencode('Halo Rajawali Motor, saya ingin booking servis injeksi...') }}" target="_blank" rel="noopener" aria-label="Chat via WhatsApp"
...
<nav class="fixed top-0 w-full z-50 glass-nav shadow-sm border-b border-outline-variant/20">
    <div class="flex justify-between items-center px-4 sm:px-8 py-3.5 max-w-7xl mx-auto">
        ...
        <div class="hidden lg:flex items-center gap-8 font-medium text-sm text-secondary">
            ...
            <a class="text-primary font-bold border-b-2 border-primary pb-0.5" href="{{ route('layanan.injeksi') }}">Injeksi</a>
        ...
        <div class="hidden sm:flex items-center gap-3">
            <a href="https://wa.me/{{ $toko['whatsapp'] }}?text={{ urlencode('Halo Rajawali Motor, saya ingin booking servis injeksi...') }}" target="_blank" rel="noopener"
...
<nav class="fixed inset-y-0 left-0 w-80 max-w-[85vw] rounded-r-2xl bg-surface z-[70] flex flex-col h-full p-6 -translate-x-full transition-transform duration-300 shadow-2xl overflow-y-auto" id="drawer">
    ...
    <div class="flex flex-col gap-2">
        ...
        <a class="flex items-center gap-3.5 p-3.5 bg-primary-container text-white font-semibold rounded-xl shadow-md" href="{{ route('layanan.injeksi') }}">
            <span class="material-symbols-outlined text-white">precision_manufacturing</span> Spesialis Injeksi
        </a>
    ...
    <div class="mt-auto pt-6 border-t border-outline-variant/30 flex flex-col gap-3">
        <a href="https://wa.me/{{ $toko['whatsapp'] }}?text={{ urlencode('Halo Rajawali Motor, saya ingin booking servis injeksi...') }}" target="_blank" rel="noopener" class="efek-kilau w-full bg-[#25D366] text-white text-center py-3.5 rounded-xl font-bold text-sm shadow-lg flex items-center justify-center gap-2 hover:bg-[#20bd5a] transition-all">
...
<main class="pt-20">
    <section class="relative min-h-[70vh] flex items-center overflow-hidden">
        <div class="absolute inset-0 z-0">
            <div class="w-full h-full bg-cover bg-center" style="background-image: url('https://lh3.googleusercontent.com/aida-public/AB6AXuCHsU1eJAN9KMHP6xzdRGPBq2tL8MGjXarxX08ki3vw8AGgrPejz5PI8FLH7WD_f_3aewFOErCdQ_TOwZj68UWWIZOHC997hW904caWGuW6pDWfqh7k3r_84HoD05yek-NgYysyGz3tmTDdyfvcXIHx1zyHK96BhO6lLMBlNscoO4n4_H25q9zIDfWSEdD4qXWf6bnagQH9Bwa-SCtinPtsLnJbh0ozrXKN6lqTK5wcR_HfQrYF8vEaBIe2TokadEgXDvCq6HO7gUk')"></div>
        ...
        <div class="relative z-10 px-4 sm:px-8 max-w-7xl mx-auto w-full py-16 sm:py-24 text-white">
            <div class="max-w-3xl space-y-6" data-reveal>
                <span class="inline-flex items-center gap-2 bg-primary-container px-4 py-1.5 rounded-full font-bold text-xs uppercase tracking-widest text-white">
                    <span class="material-symbols-outlined text-sm">precision_manufacturing</span> Specialised Injeksi Center
                </span>
                <h1 class="text-3xl sm:text-5xl lg:text-6xl font-extrabold leading-tight">Pusat Servis Injeksi Ultrasonik Surabaya</h1>
                <p class="text-white/80 text-base sm:text-xl max-w-2xl leading-relaxed">Pengisian debit bahan bakar presisi, pembersihan micro-deposit injektor, dan perbaikan respon akselerasi mesin.</p>
                <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-4 pt-4">
                    <a href="https://wa.me/{{ $toko['whatsapp'] }}?text={{ urlencode('Halo Rajawali Motor, saya ingin booking servis injeksi...') }}" target="_blank" rel="noopener"
                       class="efek-kilau bg-primary-container text-white px-8 py-4 rounded-xl font-bold text-base hover:bg-rajawali-dark transition-all shadow-xl text-center active:scale-95">
                        Booking Servis Injeksi Sekarang
                    </a>
                </div>
            </div>
        </div>
    ...
    <section class="py-20 px-4 sm:px-8 bg-surface-container-low">
        <div class="max-w-7xl mx-auto">
            <div class="text-center max-w-2xl mx-auto mb-16" data-reveal>
                <p class="text-primary font-bold text-xs uppercase tracking-widest mb-2">PAKET SERVIS INJEKSI</p>
                <h2 class="font-bold text-2xl sm:text-4xl text-on-surface">Paket Pilihan Servis Injeksi</h2>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8 items-stretch" data-reveal-stagger>
                <div data-reveal-item class="kartu-angkat efek-kilau bg-white p-8 rounded-3xl border border-outline-variant/40 shadow-sm flex flex-col justify-between">
                    <div>
                        <div class="flex justify-between items-start mb-6">
                            <div>
                                <h3 class="font-bold text-2xl text-on-surface">Clean</h3>
                                <p class="text-secondary text-xs mt-1">Pembersihan Dasar</p>
                            </div>
                            <div class="text-right">
                                <span class="text-secondary text-xs line-through">Rp 250rb</span>
                                <p class="text-primary font-bold text-2xl">Rp 195.000</p>
                            </div>
                        </div>
                        <ul class="space-y-3 mb-8 text-sm text-secondary">
                            <li class="flex items-center gap-2"><span class="material-symbols-outlined text-primary text-base">check</span> Injektor Cleaner Liquid</li>
                            <li class="flex items-center gap-2"><span class="material-symbols-outlined text-primary text-base">check</span> Scan ECU Standar OBD-II</li>
                        </ul>
                    </div>
                    <a href="https://wa.me/{{ $toko['whatsapp'] }}?text={{ urlencode('Saya berminat Paket Injeksi Clean') }}" target="_blank" rel="noopener"
                ...
                <div data-reveal-item class="kartu-angkat efek-kilau bg-inverse-surface p-8 rounded-3xl ring-4 ring-primary shadow-2xl relative flex flex-col justify-between">
                    ...
                    <div>
                        <div class="flex justify-between items-start mb-6 pt-2">
                            <div>
                                <h3 class="font-bold text-2xl text-white">Pro</h3>
                                <p class="text-primary-fixed-dim text-xs mt-1">Pembersihan Total Injeksi</p>
                            </div>
                            <div class="text-right">
                                <span class="text-white/60 text-xs line-through">Rp 450rb</span>
                                <p class="text-primary-fixed-dim font-bold text-2xl">Rp 349.000</p>
                            </div>
                        </div>
                        <ul class="space-y-3 mb-8 text-sm text-white/90">
                            <li class="flex items-center gap-2"><span class="material-symbols-outlined text-primary-fixed-dim text-base">verified</span> Pembersihan Ultrasonic Cleaning</li>
                            <li class="flex items-center gap-2"><span class="material-symbols-outlined text-primary-fixed-dim text-base">verified</span> Throttle Body Service & Calibration</li>
                            <li class="flex items-center gap-2"><span class="material-symbols-outlined text-primary-fixed-dim text-base">verified</span> Full Diagnostic ECU Reset Report</li>
                        </ul>
                    </div>
                    <a href="https://wa.me/{{ $toko['whatsapp'] }}?text={{ urlencode('Saya berminat Paket Injeksi Pro') }}" target="_blank" rel="noopener"
                ...
                <div data-reveal-item class="kartu-angkat efek-kilau bg-white p-8 rounded-3xl border border-outline-variant/40 shadow-sm flex flex-col justify-between">
                    <div>
                        <div class="flex justify-between items-start mb-6">
                            <div>
                                <h3 class="font-bold text-2xl text-on-surface">Ultimate</h3>
                                <p class="text-secondary text-xs mt-1">Performa & Perawatan Total</p>
                            </div>
                            <div class="text-right">
                                <span class="text-secondary text-xs line-through">Rp 750rb</span>
                                <p class="text-primary font-bold text-2xl">Rp 595.000</p>
                            </div>
                        </div>
                        <ul class="space-y-3 mb-8 text-sm text-secondary">
                            <li class="flex items-center gap-2"><span class="material-symbols-outlined text-primary text-base">check</span> Semua Fitur Paket Pro Complete</li>
                            <li class="flex items-center gap-2"><span class="material-symbols-outlined text-primary text-base">check</span> Penggantian Filter Injeksi Original</li>
                            <li class="flex items-center gap-2"><span class="material-symbols-outlined text-primary text-base">check</span> Garansi Performa 30 Hari</li>
                        </ul>
                    </div>
                    <a href="https://wa.me/{{ $toko['whatsapp'] }}?text={{ urlencode('Saya berminat Paket Injeksi Ultimate') }}" target="_blank" rel="noopener"
                       class="w-full border-2 border-primary text-primary py-3.5 rounded-xl font-bold text-center block hover:bg-primary hover:text-white transition-colors">Pilih Paket</a>
                </div>
            </div>
        </div>
