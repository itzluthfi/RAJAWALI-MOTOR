...
<head>
    ...
    <title>Layanan AC Mobil & Kabin | Rajawali Motor Surabaya</title>
    <meta name="description" content="Servis AC mobil dingin maksimal, pengisian freon R134a, flushing kompresor, dan penggantian filter kabin di Rajawali Motor Surabaya.">

    <!-- OpenGraph Social Preview Meta Tags -->
    <meta property="og:type" content="website"/>
    <meta property="og:url" content="https://rajawalimotor.com/layanan/ac-mobil"/>
    <meta property="og:title" content="Layanan AC Mobil & Kabin | Rajawali Motor Surabaya"/>
    <meta property="og:description" content="Servis AC mobil dingin maksimal, flushing kompresor digital & pengisian freon R134a di Surabaya."/>
...
<a href="https://wa.me/{{ $toko['whatsapp'] }}?text={{ urlencode('Halo Rajawali Motor, saya ingin booking servis AC mobil...') }}" target="_blank" rel="noopener" aria-label="Chat via WhatsApp"
...
<nav class="fixed top-0 w-full z-50 glass-nav shadow-sm border-b border-outline-variant/20">
    <div class="flex justify-between items-center px-4 sm:px-8 py-3.5 max-w-7xl mx-auto">
        ...
        <div class="hidden lg:flex items-center gap-8 font-medium text-sm text-secondary">
            ...
            <a class="text-primary font-bold border-b-2 border-primary pb-0.5" href="{{ route('layanan.ac-mobil') }}">AC Mobil</a>
        </div>
        <div class="hidden sm:flex items-center gap-3">
            <a href="https://wa.me/{{ $toko['whatsapp'] }}?text={{ urlencode('Halo Rajawali Motor, saya ingin booking servis AC mobil...') }}" target="_blank" rel="noopener"
...
<nav class="fixed inset-y-0 left-0 w-80 max-w-[85vw] rounded-r-2xl bg-surface z-[70] flex flex-col h-full p-6 -translate-x-full transition-transform duration-300 shadow-2xl overflow-y-auto" id="drawer">
    ...
    <div class="flex flex-col gap-2">
        ...
        <a class="flex items-center gap-3.5 p-3.5 bg-primary-container text-white font-semibold rounded-xl shadow-md" href="{{ route('layanan.ac-mobil') }}">
            <span class="material-symbols-outlined text-white">ac_unit</span> AC Mobil
        </a>
    </div>

    <div class="mt-auto pt-6 border-t border-outline-variant/30 flex flex-col gap-3">
        <a href="https://wa.me/{{ $toko['whatsapp'] }}?text={{ urlencode('Halo Rajawali Motor, saya ingin booking servis AC mobil...') }}" target="_blank" rel="noopener" class="efek-kilau w-full bg-[#25D366] text-white text-center py-3.5 rounded-xl font-bold text-sm shadow-lg flex items-center justify-center gap-2 hover:bg-[#20bd5a] transition-all">
...
<main class="pt-20">
    <section class="relative min-h-[70vh] flex items-center overflow-hidden">
        <div class="absolute inset-0 z-0">
            <div class="w-full h-full bg-cover bg-center" style="background-image: url('https://lh3.googleusercontent.com/aida-public/AB6AXuBaVBo1o87keGX7BojiuwmIQ9ZDdfukT-hvsqCFVHJOAGIhHKzJeZGna9hwH_iODo91-X979SLNps5WOMYN9phM7wC0IQQXvNnyv9nKjwgqxqOlN61lS2WwHuoHOeqQuh9xD4DskNgYJKXC_lJAGTcC_gG5nJqkUOlOTD6ZMf6Zhbz_hHBHJ3kgrz2gLup49HBltH2s0VddvozlZPilFk5o2JbXgT16dnWloGpXbg1DQUUpKBvgIgdY8Fgj6GybdL9mMG_AAbGgMbw')"></div>
        ...
        <div class="relative z-10 px-4 sm:px-8 max-w-7xl mx-auto w-full py-16 sm:py-24 text-white">
            <div class="max-w-3xl space-y-6" data-reveal>
                <span class="inline-flex items-center gap-2 bg-primary-container px-4 py-1.5 rounded-full font-bold text-xs uppercase tracking-widest text-white">
                    <span class="material-symbols-outlined text-sm">ac_unit</span> Climate Control & AC Care
                </span>
                <h1 class="text-3xl sm:text-5xl lg:text-6xl font-extrabold leading-tight">Servis AC Mobil Dingin Maksimal</h1>
                <p class="text-white/80 text-base sm:text-xl max-w-2xl leading-relaxed">Pengisian freon R134a murni, flushing oli kompresor digital, cuci evaporator, dan penggantian filter kabin bebas bau apek.</p>
                <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-4 pt-4">
                    <a href="https://wa.me/{{ $toko['whatsapp'] }}?text={{ urlencode('Halo Rajawali Motor, saya ingin booking servis AC...') }}" target="_blank" rel="noopener"
    ...
    <section class="py-20 px-4 sm:px-8 bg-surface-container-low">
        <div class="max-w-7xl mx-auto">
            <div class="text-center max-w-2xl mx-auto mb-16" data-reveal>
                <p class="text-primary font-bold text-xs uppercase tracking-widest mb-2">PAKET SERVIS AC</p>
                <h2 class="font-bold text-2xl sm:text-4xl text-on-surface">Pilih Paket Perawatan AC</h2>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8 items-stretch" data-reveal-stagger>
                <div data-reveal-item class="kartu-angkat efek-kilau bg-white p-8 rounded-3xl border border-outline-variant/40 shadow-sm flex flex-col justify-between">
                    <div>
                        <div class="flex justify-between items-start mb-6">
                            <div>
                                <h3 class="font-bold text-2xl text-on-surface">AC Refresh</h3>
                                <p class="text-secondary text-xs mt-1">Perawatan Ringan</p>
                            </div>
                            <div class="text-right">
                                <p class="text-primary font-bold text-2xl">Rp 150.000</p>
                            </div>
                        </div>
                        <ul class="space-y-3 mb-8 text-sm text-secondary">
                            <li class="flex items-center gap-2"><span class="material-symbols-outlined text-primary text-base">check</span> Pengisian Freon R134a Murni</li>
                            <li class="flex items-center gap-2"><span class="material-symbols-outlined text-primary text-base">check</span> Pembersihan Filter Kabin</li>
                            <li class="flex items-center gap-2"><span class="material-symbols-outlined text-primary text-base">check</span> Fogging Anti Bakteri Kabin</li>
                        </ul>
                    </div>
                    <a href="https://wa.me/{{ $toko['whatsapp'] }}?text={{ urlencode('Saya berminat Paket AC Refresh') }}" target="_blank" rel="noopener"
                ...
                <div data-reveal-item class="kartu-angkat efek-kilau bg-inverse-surface p-8 rounded-3xl ring-4 ring-primary shadow-2xl relative flex flex-col justify-between">
                    ...
                    <div>
                        <div class="flex justify-between items-start mb-6 pt-2">
                            <div>
                                <h3 class="font-bold text-2xl text-white">AC Double Blower</h3>
                                <p class="text-primary-fixed-dim text-xs mt-1">Flushing Kompresor + Evaporator</p>
                            </div>
                            <div class="text-right">
                                <p class="text-primary-fixed-dim font-bold text-2xl">Rp 280.000</p>
                            </div>
                        </div>
                        <ul class="space-y-3 mb-8 text-sm text-white/90">
                            <li class="flex items-center gap-2"><span class="material-symbols-outlined text-primary-fixed-dim text-base">verified</span> Flushing Oli Kompresor Digital</li>
                            <li class="flex items-center gap-2"><span class="material-symbols-outlined text-primary-fixed-dim text-base">verified</span> Cuci Evaporator & Blower AC</li>
                            <li class="flex items-center gap-2"><span class="material-symbols-outlined text-primary-fixed-dim text-base">verified</span> Penggantian Filter Kabin Carbon Baru</li>
                        </ul>
                    </div>
                    <a href="https://wa.me/{{ $toko['whatsapp'] }}?text={{ urlencode('Saya berminat Paket AC Double Blower') }}" target="_blank" rel="noopener"
                ...
                <div data-reveal-item class="kartu-angkat efek-kilau bg-white p-8 rounded-3xl border border-outline-variant/40 shadow-sm flex flex-col justify-between">
                    <div>
                        <div class="flex justify-between items-start mb-6">
                            <div>
                                <h3 class="font-bold text-2xl text-on-surface">Overhaul Kompresor</h3>
                                <p class="text-secondary text-xs mt-1">Perbaikan Total Sistem AC</p>
                            </div>
                            <div class="text-right">
                                <p class="text-primary font-bold text-2xl">Rp 550.000</p>
                            </div>
                        </div>
                        <ul class="space-y-3 mb-8 text-sm text-secondary">
                            <li class="flex items-center gap-2"><span class="material-symbols-outlined text-primary text-base">check</span> Bongkar & Servis Kompresor AC</li>
                            <li class="flex items-center gap-2"><span class="material-symbols-outlined text-primary text-base">check</span> Ganti Seal O-Ring Tahan Kebocoran</li>
                            <li class="flex items-center gap-2"><span class="material-symbols-outlined text-primary text-base">check</span> Cek Kebocoran UV Leak Detector</li>
                            <li class="flex items-center gap-2"><span class="material-symbols-outlined text-primary text-base">check</span> Garansi Dingin 30 Hari</li>
                        </ul>
                    </div>
                    <a href="https://wa.me/{{ $toko['whatsapp'] }}?text={{ urlencode('Saya berminat Paket Overhaul Kompresor') }}" target="_blank" rel="noopener"
                       class="w-full border-2 border-primary text-primary py-3.5 rounded-xl font-bold text-center block hover:bg-primary hover:text-white transition-colors">Pilih Paket</a>
                </div>
            </div>
        </div>
